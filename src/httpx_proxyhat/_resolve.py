"""Gateway credential resolution for httpx-proxyhat.
    ...
    *,
    ...
    if not api_key:
        raise ProxyHatConfigError(
            "httpx-proxyhat: no credentials. Pass api_key (or PROXYHAT_API_KEY), "
...
def _resolve_sub_user(api_key: str, want: str | None) -> tuple[str, str]:
    ...
    if chosen is None or not chosen.proxy_username or not chosen.proxy_password:
        raise ProxyHatConfigError(
            f'httpx-proxyhat: no sub-user matched "{want}" (or it has no proxy credentials).'
            if want
            else "httpx-proxyhat: no usable sub-user found (all suspended or out of traffic). "
